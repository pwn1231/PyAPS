...
def MERRAdload(bdate,hr,filedir, hdf=False):
    ...
    for i in xrange(len(bdate)):
        ...
        hr = hr
        year = int(yr)
        url1 = 'http://goldsmr5.gesdisc.eosdis.nasa.gov/daac-bin/OTF/HTTP_services.cgi?FILENAME=%2Fdata%2Fs4pa%2FMERRA2%2FM2I6NPANA.5.12.4%2F'
        url2 = '%2F'
        ...
        if not os.path.exists(dir):
            ...
            dloadCmd = 'wget "{}" --user {} --password {} --load-cookies ~/.urs_cookies --save-cookies ~/.urs_cookies --auth-no-challenge=on --keep-session-cookies -O {}'.format(weburl, user, pw, filename)
            os.system(dloadCmd)

    return flist
                                
def MERRA1dload(bdate,hr,filedir, hdf=True):
    print 'By using "MERRA1dload" you have chosen to exclusively download MERRA-1 data. However, if any date in their workspace is on or after March 1st, 2016, the program will abort as there are no available MERRA-1 datasets for this time period.'
    flist = []
    for i in xrange(len(bdate)):
        date = bdate[i]
        if int(date) >= 20160301:  ###SSS 3/2016: If we are looking at dates after March 1st, 2016, then only MERRA-2 nc4 files are available
            print 'MERRA-1 models after March 1st, 2016 not available, one or more scenes in your workspace are on or after this date'
            sys.exit(1)

    for i in xrange(len(bdate)):
        date = bdate[i]
        if int(date) < 20160301:  ###SSS 3/2016: If we are looking at dates before March 1st, 2016, then MERRA-1 hdf files are still available
...
def MERRA2dload(bdate,hr,filedir, hdf=False): ###Same as "MERRAdload" above
    flist = []
    for i in xrange(len(bdate)):
        date = bdate[i]
        filename = '%s/merra-%s-%s.nc4' %(filedir,date,hr)
