...
class PyAPS_rdr:
	...
	def __init__(self,gribfile,demfile,grib='ERA5',humidity='Q',demtype=np.float32,demfmt='RMG',verb=False, Del='comb'):
		...
		assert grib in ('ERA5','NARR','ECMWF','MERRA'), 'PyAPS: Undefined grib file source.'
		self.grib = grib
		...
		if self.grib in ('ERA5'):
			[lvls,latlist,lonlist,gph,tmp,vpr] = era.get_ecmwf('ERA5',self.gfile,self.minlat,self.maxlat,self.minlon,self.maxlon,self.dict,humidity=self.humidity,verbose=verb)
